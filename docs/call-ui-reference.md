...
- `CONNECTED`：目标 `subscribe` 收到音频或视频任一真实 track 后显示媒体层和通话控制；仅有音频时视频区域显示无画面状态。
...
- 音频通话仍须收到目标 `subscribe` 音频 track；视频通话收到目标 `subscribe` 音频或视频任一真实 track 即可接通，不强制等待视频 track。
- 仅收到音频时保留无视频画面状态；后续视频 track 到达后更新 `hasRemoteVideo` 并绑定 renderer，不重新改变业务接听流程。
- 不得用空的本地 video track、黑帧或虚假 `mediaConnected=true` 掩盖摄像头失败。
