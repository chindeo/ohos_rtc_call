...
当 OpenHarmony Network Kit 无法解析、但系统 libc 可以解析同一域名时，宿主可改用公共包提供的原生 curl 请求适配器。

- 保留现有请求拦截器对 `baseURL`、请求头和重试字段的修改能力。
- GET 查询参数、JSON POST/PUT 请求体、连接超时和总超时行为保持不变。
- JSON 响应自动解析，非 JSON 响应保留字符串。
- 非 2xx 响应和 libcurl 错误以附带原始请求配置的错误对象拒绝 Promise。
- HAR 同时包含 `armeabi-v7a` 和 `arm64-v8a` 原生库。
- API 23 问题设备配置域名后，配置请求和 `/license` 请求不再出现 Network Kit 错误 `2300006`。
- 公共 HAR 接入后，床旁和主机的 API 12、API 23 包均须重新构建归档。

## WebSocket 断线重连验收

- 默认重连策略为：初始退避 5 秒、指数倍数 2、最大 5 分钟、设备固定分片窗口 30 秒、连接和注册超时各 15 秒、连续注册稳定 60 秒后清零失败次数。
- 每次重连延迟由指数退避区间内的随机抖动和设备身份哈希固定偏移共同组成，避免大量设备在网络恢复时同时冲击网关。
- WebSocket `error` 和 `close` 连续到达只能产生一个重连任务；每次重连必须创建新的 socket，旧 socket 回调不能改变新连接状态。
- 主动 `close()`、宿主 controller `dispose()`、组件销毁或地址切换必须取消旧重连、超时和稳定计时器。
- 信令离线时注册状态必须立即变为离线并清除旧 server 信息，但不得清理仍在工作的媒体会话；重新注册后待发送的拨打或接听操作只能补发一次。
- 240 真机验收使用只针对 WebRTC 网关地址的临时 blackhole host route，最长保留 20 秒，并确保无论验证成功或失败都删除该路由；不得断开整机网络、HDC 或 MQTT 链路。
- 删除临时路由后无需重启应用，设备应在 5 分钟内重新出现在 WebRTC 后台在线列表，日志应包含重连次数、计划延迟和注册恢复结果。
...
- 主机和床旁的视频通话在对应 `uid + rtcUID + subscribe` 收到真实音频或视频任一 track 后进入“通话中”，不能继续强制等待视频 track。
- 视频暂未到达但音频已到达时，`mediaConnected=true`、`hasRemoteAudio=true`、`hasRemoteVideo=false`；通话控制保持可用，后续真实视频 track 到达后必须更新 `hasRemoteVideo` 并正常显示画面。
- 本地 `publish connected`、ICE connected、call-gateway `c__answer` 或其它会话的 track 不能代替目标 `subscribe` 的真实媒体。
- 媒体回调必须按 callId、uid、sessionKey 或唯一目标号码归一；归一失败时不得把其它通话推进为已接听。不能注入黑帧或保留无输入的假 live 轨道冒充媒体正常。
...
- 主机点击接听后先显示连接中；收到对应 `subscribe` 音频或视频任一真实 track 后显示通话中。只有音频时视频区域保持无画面，后续视频到达后继续绑定 renderer。
...
- API23 UVC 打开前必须检查临时 USB 权限，缺少权限时调用 `requestRight`；日志必须包含失败阶段及 error name/code/message，不能只输出 `{}`。
- API23 UVC 打开失败时执行有限次数重试；全部失败或运行中拔出设备后，必须停止并关闭 UVC、用 CameraKit track 替换现有 publish sender。CameraKit 再失败时移除失败 video track，不能保留“假 live”的空视频轨道；音频和其它 subscribe 会话不得中断。
