...
| `CONNECTED` | 目标 `subscribe` 收到音频或视频任一真实 track 后显示通话页 |
| `HELD` | 只更新目标通话的保持头像和恢复按钮 |

接通判定不能使用以下事件代替目标 `subscribe` 的真实媒体：
...
普通视频通话匹配到目标音频或视频任一 track 即可接通，不再等待视频；仅有音频时显示无画面状态，后续视频到达后仍须绑定 renderer。媒体状态回调中的 `sessionKey`、`sessionId`、`uid` 和目标号码会归一到同一路业务会话；归一失败时不得伪造接通。
...
2. 点击接听后先显示连接中；目标音频到达而视频无数据时也应显示通话页，后续视频到达后正常显示画面。
3. 多路呼入两列网格及奇数末项跨列。
4. 多路主画面切换和右侧独立视频缩略图。
